...
export default function ItemSale({item, onPress}) {
  ...
  return (
    <Pressable style={{borderBottomColor: '#d9d9d9', borderBottomWidth: 1, paddingVertical: 10, backgroundColor: isSelected && 'rgba(35,102,203,0.5)' }} onPress={()=>addDeleteSale(item)} >
      ...
      <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
        <Text style={{fontSize: 12, fontFamily: 'Cairo-Regular', color: '#7F8487'}}>{item?.cliente.direccion.calle} {item?.cliente.direccion.numero} , {item?.cliente.direccion.ciudad}</Text>
