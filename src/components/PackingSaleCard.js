...
export default function PackingSaleCard({length}) {
  ...
  return (
    <View>
        ...
        <Text style={{fontSize: 14, fontFamily: 'Cairo-Regular', color: '#7F8487', marginStart: 15}}>Direccion:   {saleActive?.cliente.direccion.calle} {saleActive?.cliente.direccion.numero}, {saleActive?.cliente.direccion.ciudad}</Text>
        <View style={{borderBottomColor: '#d9d9d9', borderBottomWidth: 1, marginHorizontal: '20%', marginTop: 10, marginBottom: 5}}/>
        <Text style={{fontSize: 14, fontFamily: 'Cairo-Regular', color: '#9E9E9E', marginStart: 15 }}>Categorias:</Text>
        ...
        <View style={{paddingHorizontal: 15}} >
          ...
          <SwipeListView
            data={filterSale}
            style={[styles.list, {paddingHorizontal: 10, height: '50%', marginVertical: 5}]}
            renderItem={
                ({item})=>(
                    <TouchableHighlight
            ...
            renderHiddenItem={({item})=><RenderHiddenItem item={item} rowClose={rowClose} editQty={editQty} deleteItemCart={()=>console.log("delete")} />}
            rightOpenValue={-200}
            leftOpenValue={78}
            previewRowKey={'0'}
            previewOpenValue={-40}
        ...
        <Text style={{fontSize: 14, fontFamily: 'Cairo-Regular', color: '#9E9E9E', marginEnd: 15, textAlign: 'right', marginTop: 5 }}>{qtyMiss.length} agregado de {saleActive?.lineaVenta.filter(item=>item.estado !== "entregado").length}</Text>
        <Text style={{fontSize: 14, fontFamily: 'Cairo-Regular', color: '#9E9E9E', marginEnd: 15, textAlign: 'right' }}>Total: ${saleActive?.total}</Text>
    </View>
  ) 
}
